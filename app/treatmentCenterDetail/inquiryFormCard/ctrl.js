function ctrl($log, $rootScope, Status, TreatmentCenterService) {
  ...
  vm.testNum = function () {
    if (angular.isDefined(vm.phone)) {
      ...
      if (len > 3) {
        ...
        if ((usCodes.indexOf(parseInt(code, 10)) === -1)) {
          vm.phoneValidate = 0;
          vm.inquiryForm.phone.$error.pattern = true;
          vm.inquiryForm.phone.$valid = false;
          vm.inquiryForm.phone.$invalid = true;
        } else {
          vm.phoneValidate = 1;
          vm.inquiryForm.phone.$error.pattern = false;
          vm.inquiryForm.phone.$valid = true;
          vm.inquiryForm.phone.$invalid = false;
        }
      }
    }
    ...
    if (vm.phoneValidate) {
      ...
      vm.inquiryForm.phone.$invalid = false;
      vm.inquiryForm.phone.$error.pattern = false;
      vm.inquiryForm.phone.$error.minlength = false;
      vm.inquiryForm.phone.$error.maxlength = false;
    }
  };
  vm.submit = function () {
    var data = {
