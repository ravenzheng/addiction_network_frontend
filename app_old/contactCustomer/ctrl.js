...
function ctrl(Status, $log, $rootScope, service, $window, $state, UIState) {
  // initialize
  var vm = this;
  $rootScope.title = 'Contact Customer';
  $rootScope.description = 'Contact Customer';
  ...
  vm.submit = function () {
    ...
    service.sendMessage(data).then(function (response) {
      ...
      $log.error(err);
      vm.clearForm();
      $state.go(UIState.CONTACTCUSTOMERTHANK);
      // $window.location.href = '/contact-customer-thank';
